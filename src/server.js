...
app.post('/api/control', (req, res) => {
    const { element, state } = req.body; // Riceve i dati da n8n
    
    if (!element || !state) {
        return res.status(400).send({ message: "Dati mancanti: element o state" });
    }
    
    console.log(`Comando ricevuto: Oggetto: ${element}, Azione: ${state}`);
    // Inoltra il comando a tutti i client (browser) connessi
    io.emit('3d_command', { element, state }); // <--- QUESTO È IL PASSAGGIO CHIAVE
    ...
    // Esempio: io.emit('3d_command', { element, state });

    // Risposta a n8n per confermare la ricezione
    res.status(200).send({ 
        message: `Comando per ${element} in stato ${state} ricevuto e inoltrato.` 
    });
});
